    ...
    function fn(absCurrentDirPath) {
      ...
      items.forEach((itemName) => {
        ...
        for (let i = 0; i < methodsConfig.length; i++) {
          ...
          if (!methodsConfig[i].options.cache) {
            methodsConfig[i].options.cache = {
              expiresIn: 24 * HOUR,
              generateTimeout: 3 * MINUTE,
            };
          }
          if (!methodsConfig[i].options.bind) {
            methodsConfig[i].options.bind = {};
          }
          if (!methodsConfig[i].options.bind.server) {
            methodsConfig[i].options.bind.server = server;
          }
